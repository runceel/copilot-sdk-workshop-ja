...
> パーミッション種別のみを 1 リクエストずつ承認しますが、正確なターゲットを強制することはできません。
> このフラグは、ワークショップ用の使い捨てローカルデモでのみ使用し、本番環境、共有環境、信頼できない URL では
> 決して使用しないでください。
:::
ワークショップのターゲットを使用します。
...
このワークショップ用のサンプルページには、ブラウザーで確認できる問題が意図的に含まれています。テキスト代替の欠落、
