...
この例ではパーミッションハンドラー以外の設定を変更していないため、セッションでは SDK の既定のペルソナが使われます。ペルソナを設定する項目が、[system message（システムメッセージ）](https://github.com/github/copilot-sdk/blob/main/docs/getting-started.md#customize-the-system-message)です。システムメッセージには、次の 3 つの指定方法があります。

- `append`（既定）: 指定した内容を SDK が用意するプロンプトに追加します。環境情報、ツールの使い方、セキュリティ上の制約といった SDK の設定や、既定の CLI ペルソナはそのまま維持されます。
- `replace`: SDK のプロンプト全体を、指定した内容に置き換えます。
- `customize`: トーン、ガイドライン、コード変更ルールなど、指定した項目だけを変更し、それ以外は維持します。

このワークショップではシステムメッセージを変更しないため、以降の回答には既定の CLI ペルソナが使われます。アプリ独自の語り口やルールを設定する場合は、`replace` または `customize` を使います。
